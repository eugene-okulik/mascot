...
student_id_query = "SELECT id FROM students WHERE name = %s AND second_name = %s"
...
cursor.executemany("INSERT INTO subjets (title) VALUES (%s)",
                   [('Not for losers',), ('Pretty easy programming',), ('What is DB?',)])

cursor.executemany("INSERT INTO lessons (title, subject_id) SELECT %s, id FROM subjets WHERE title = %s",
                   [('1st lesson', 'Not for losers'), ('2nd lesson', 'Not for losers'),
                    ('1st lesson', 'Pretty easy programming'), ('2nd lesson', 'Pretty easy programming'),
                    ('1st lesson', 'What is DB?'), ('2nd lesson', 'What is DB?')])

lesson_id_query = "SELECT id FROM lessons WHERE title = %s AND subject_id IN (SELECT id FROM subjets WHERE title = %s)"
...
cursor.executemany("INSERT INTO marks (value, lesson_id, student_id) VALUES (%s, %s, %s)",
                   [('Great!', lesson_id, student_id), ('Good', lesson_id, student_id),
                    ('Not Good', lesson_id, student_id), ('Bad', lesson_id, student_id),
                    ('Terrible', lesson_id, student_id), ('Ok', lesson_id, student_id)])
